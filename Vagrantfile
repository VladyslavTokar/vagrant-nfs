...
## NFS Server-1
    ...
    nfs_srv.vm.network "private_network", ip: "192.168.51.2",
     virtualbox__intnet: "sw1"
    nfs_srv.vm.hostname = "SRV1"
    # Provider
	...
	firewall-cmd --reload
	for service in {rpc,nfsd}; do
          echo 'configuring debug for $service'
          for param in {-s,-c}; do
            rpcdebug -m $service $param all
          done
	done
...
## NFS Client-1
 config.vm.define 'nfs_cl1' do |nfs_cl1|
  ...
  # Create a private network
  nfs_cl1.vm.network "private_network", ip: "192.168.51.2",
   virtualbox__intnet: "sw1"
  ...
  nfs_cl1.vm.provider "virtualbox" do |vb|
   ...
   config.vm.provision "shell", inline: <<-SHELL
	...
	grep '192.168.51.1:/var/nfsfileshare' /etc/fstab >/dev/null
	if [ $? -ne 0 ]; then
	  echo 'mounting nfsfileshare'
	  echo "192.168.51.1:/var/nfsfileshare/ /mnt/nfsfileshare nfs rw,sync,hard,intr 0 0" >> /etc/fstab
          mount -a
	 else
...
## NFS Client-2
 config.vm.define 'nfs_cl2' do |nfs_cl2|
  ...
  # Create a private network
  nfs_cl2.vm.network "private_network", ip: "192.168.51.3",
   virtualbox__intnet: "sw1"
